...
class touTest():
  ...
  def touConfig(self): #write predefined schdeule, season, and holiday setting to meter; return 1 if no conflict in settings. 
    ...
    if(self.syncConnectRead(30734,1) != 0):
        return False
    else:
        return True
    
  def checkTouEnergy(self,TariffIndex:int,sleeptime:int = 60):
    ...
    energyAccumulated = (end_reading-start_reading)/10 #Assume default 0.1 primary reading
    expectedEnergy = 60*energyAccumulated*(60/sleeptime) # get expect accumulated energy
    if(expectedEnergy-18000)>100: #TO BE MODIFIED, depends on the requirement
  ...
  def writeSingleDayClock(self,date): #Simulate a single day, 
    print('On date: ',date)
    thisTime = date
    ...
    currDate[3] = 9 # subject to change
  ...
  #Depending on wheather presentDate is None or a list, this function will simulate a whole year's TOU or a single day schedule
  def smartClock(self,presentDate:list =None):
    if(not presentDate): #check whole year's schedule
      ...
      while(self.specialDate): # if special date remains
          thisTime = self.specialDate[0]
          self.specialDate = self.specialDate[1:] # popleft, update the list
          self.writeSingleDayClock(thisTime)
          
      while(self.seasonList): # if season remains
          thisTime = self.seasonList[0]
          self.seasonList = self.seasonList[1:] # popleft, update the list
          self.writeSingleDayClock(thisTime)
...
def meter_instance():
    host = "192.168.60.110"
    meterBasic = touTest(host)
    return meterBasic

meter = meter_instance() #declare a global meter class

def test_touConfig():
  assert meter.touConfig() == True
    
def test_touRunOneYear():
  assert meter.smartClock() == True
  
def test_touRunOneDay():
  assert meter.smartClock([1,1,14]) == True